...
model = SwinUNETR(img_size=(224, 224, 96), in_channels=4, out_channels=3, feature_size=24).to(device)
model.load_state_dict(torch.load("ssmodelsave/swin_best_metric_model.pth", map_location='cuda:0'))
...
train_ds = BratsDataset(datadict, train_transform)
train_loader = DataLoader(train_ds, batch_size=1)
val_ds = BratsDataset(datadict[0:100], val_transform)
...
if __name__ == '__main__':
    for epoch in range(max_epochs):
        ...
        torch.save(
            model.state_dict(),
            os.path.join("ssmodelsave", "swin_best_metric_model.pth"),
        )
